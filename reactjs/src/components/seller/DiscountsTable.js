...
const DiscountsTable = () => {
  ...
  async function fetchDiscounts() {
    try {
      ...
      setDiscounts(
        response.data.filter((discount) =>
          dates.inRange(new Date(), discount.dateFrom, discount.dateTo)
        )
      );
    } catch (error) {
  ...
  async function deleteDisc(id) {
    try {
      ...
      console.error(
        `Error ocurred while deleting discount with id ${id}: ${error}`
      );
    }
  }
            ...
            <tr>
              ...
              <td>
                <Button
                  variant="contained"
                  color="primary"
                  onClick={() => goToTaskEditPage(disc.id)}
                >
                  Modify
                </Button>
              </td>
